...
export default function AgencyTransactionsPage() {
    ...
    return (
        <div className="space-y-8">
            <PageHeader
                title="Transactions"
                subtitle="Manage individual booking transactions and payments"
                breadcrumbs={['Agency', 'Transactions']}
            />
            ...
            <div className="glass-panel p-4 flex flex-col md:flex-row gap-4 items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    <Badge variant="info">{pagination.total} Total Transactions</Badge>
                </div>
            </div>
            ...
            <div className="glass-panel p-8">
                {filteredTransactions.length > 0 ? (
                        ...
                        {filteredTransactions.map((tx: any) => (
                            <div key={tx._id} className={`flex items-center gap-4 py-2 px-4 bg-white/60 border-l-4 rounded-xl hover:bg-white/90 transition-all text-sm group shadow-sm hover:shadow-md ${tx.status === 'CONFIRMED' ? 'border-l-success-500' : 'border-l-danger-500'}`}>
                                <div className="flex-[0.8] min-w-0">
                                    <div className="text-[10px] font-black text-neutral-400 uppercase tracking-tighter mb-0.5">Chronicle</div>
                                    <div className="font-medium text-neutral-600 text-xs">{new Date(tx.createdAt).toLocaleDateString()}</div>
                                </div>

                                <div className="flex-1 min-w-0">
                                    <div className="text-[10px] font-black text-neutral-400 uppercase tracking-tighter mb-0.5">Protocol Ref</div>
                                ...
                                <div className="flex-[2] min-w-0 hidden md:block">
                                    <div className="text-[10px] font-black text-neutral-400 uppercase tracking-tighter mb-0.5">Assigned Passenger</div>
                                    <div className="flex items-center gap-2">
                                        <div className="font-bold text-neutral-800 truncate">{tx.passengers?.[0]?.name || 'Booking'}</div>
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="text-[10px] font-black text-neutral-400 uppercase tracking-tighter mb-0.5">Settlement</div>
                                ...
                                <div className="flex items-center gap-3 ml-auto">
                                    <div className="flex flex-col items-end gap-0.5">
                                        <Badge variant={tx.status === 'CONFIRMED' ? 'success' : 'danger'} size="sm" className="scale-[0.85] origin-right">
                                            {tx.status}
                                        </Badge>
                                        <Badge variant={
                                            tx.paymentStatus === 'PAID' ? 'success' :
                                                tx.paymentStatus === 'REFUNDED' ? 'info' : 'warning'
                                        } size="sm" className="scale-[0.85] origin-right">
                                            {tx.paymentStatus}
                                        </Badge>
                                    </div>
                    ...
                    <EmptyState
                        ...
                        description={search ? "We couldn't find any transactions matching your search." : "Your transaction history will appear here once bookings are made."}
                    />
                )}
            </div>
