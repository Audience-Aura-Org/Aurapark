...
export default function TripsPage() {
    ...
    if (loading) return (
        <div className="min-h-screen bg-neutral-50">
            <Sidebar />
            <main className={`transition-all duration-500 min-h-screen pt-20 safe-bottom-nav ${user ? (isCollapsed ? 'lg:pl-20' : 'lg:pl-72') : ''}`}>
            ...
            <div className="text-center">
                <p className="text-lg font-bold text-neutral-800 tracking-tight">Loading Trips</p>
                <p className="text-sm text-neutral-500 font-medium">Fetching available routes...</p>
                </div>
            </div>
            </main>
        </div>
    ...
    return (
        <div className="min-h-screen bg-neutral-50">
            <Sidebar />
            <main className={`transition-all duration-500 pt-20 pb-20 px-4 md:px-8 relative overflow-hidden safe-bottom-nav ${user ? (isCollapsed ? 'lg:pl-20' : 'lg:pl-72') : ''}`}>
            ...
            <div className="max-w-6xl mx-auto space-y-10 relative z-10">
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center gap-3 text-sm font-semibold text-neutral-500">
                            <span className="flex items-center gap-1.5 bg-white px-3 py-1 rounded-full border border-neutral-200 shadow-sm">
                                <svg className="w-4 h-4 text-neutral-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /></svg>
                                {trips.length} Trips Available
                            </span>
                        </div>
                        <h1 className="text-4xl md:text-5xl font-black text-neutral-900 leading-tight tracking-tight">
                            Browse All <span className="text-gradient-green">Trips</span>
                        </h1>
                        <p className="text-base text-neutral-600 font-medium">
                            View all upcoming trips. No search required — book directly or filter by route.
                        </p>
                    </div>
                    <Link href="/">
                        <Button variant="primary" className="md:px-6 py-3 flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 gap-6">
                    {trips.map((trip, index) => {
                        ...
                        const origin = trip.routeId?.stops?.[0]?.name || 'Origin';
                        const destination = trip.routeId?.stops?.[trip.routeId?.stops?.length - 1]?.name || 'Destination';
                                ...
                                <div className="p-6 md:p-8 flex flex-col lg:flex-row gap-8 lg:gap-10">
                                        ...
                                        <div className="mt-6 space-y-3">
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {trip.busId?.amenities?.map((a: string) => (
                                                    <span key={a} className="inline-flex items-center gap-1 text-[10px] font-medium text-neutral-500 bg-neutral-50 px-2 py-1 rounded border border-neutral-100">
                                                        {getAmenityIcon(a)}
                                                        {a}
                                                    </span>
                                                ))}
                                    ...
                                    <div className="flex-1 border-t lg:border-t-0 lg:border-l border-neutral-100 pt-6 lg:pt-0 lg:pl-10 flex flex-col justify-center">
                                        <div className="flex items-center justify-between gap-4">
                                            <div className="text-left min-w-[120px]">
                                                <div className="text-[10px] font-medium text-primary-600 mb-1">
                                            ...
                                            <div className="text-right min-w-[120px]">
                                                ...
                                                <div className="flex items-center justify-end gap-1.5 text-[11px] font-medium text-primary-700">
                                    ...
                                    <div className="lg:w-[220px] flex flex-row lg:flex-col items-center justify-between lg:justify-center border-t lg:border-t-0 lg:border-l border-neutral-100 pt-6 lg:pt-0 lg:pl-10">
                                        <div className="text-right lg:text-center w-full">
                                            ...
                                            <p className="text-[10px] text-neutral-400 font-medium mt-1">Per passenger</p>
                                        </div>
                ...
                {trips.length === 0 && !loading && (
                    <div className="bg-white rounded-3xl p-16 text-center border border-dashed border-neutral-300">
                        ...
                        <h2 className="text-xl font-bold text-neutral-900 mb-2">No Trips Available</h2>
                        <p className="text-neutral-500 max-w-sm mx-auto mb-8">
                            There are no upcoming trips at the moment. Try searching for a specific route.
                        </p>
                        <Link href="/">
                            <Button variant="primary">Search Routes</Button>
                        </Link>
                    </div>
                )}
            </div>
